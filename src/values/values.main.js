...
class Values extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = {
            values: [{name: 'hello world', id: 1}, {name: "wornderful", id: 2}],
            detail: {},
            editing: false,
            updating: false
        }
...
// better handling is needed
        ...
        if (this.state.updating) {
            axios({
                method: "put",
                url: `${hostUri}/values/${this.state.detail.id}`,
                responseType: "application/json",
    ...
    render() {
        return (<div>
                <Grid container spacing={24}>
                    <Grid item xs={6}>
                        <ValuesList values={this.state.values} select={this._selectDetail}/>
                        <Button variant="fab" color="primary" aria-label="add" onClick={this._newOpp}>
                            <AddIcon/>
                        </Button>
                    </Grid>
                    <Grid item xs={6}>
                        <ValuesDetail detail={this.state.detail} handleChange={this._handleChange}
